---
title: "Kimchijigae"
excerpt: "Best use of old kimchi"
date: "2020-11-15"
categories: ["vegan"]
tags: ["korean", "winter"]
difficulty: "1"
duration: "1"
ingredients:
  [
    "gochugaru",
    "gochujang",
    "green-onion",
    "kelp",
    "kimchi",
    "onion",
    "seasame-oil",
    "tofu",
  ]
url: "https://www.maangchi.com/recipe/kimchi-jjigae"
coverImage: "https://upload.wikimedia.org/wikipedia/commons/a/a7/Kimchijigae.jpg"
---

## Ingredients

For 4 servings:

- 1 pound kimchi, cut into bite size pieces
- ¼ cup kimchi brine
- Optional: ½ pound pork shoulder (or pork belly)
- ½ package of tofu (optional), sliced into ½ inch thick bite size pieces
- 3 green onions
- 1 medium onion, sliced (1 cup)
- 1 teaspoon kosher salt
- 2 teaspoons sugar
- 2 teaspoons gochugaru (Korean hot pepper flakes)
- 1 tablespoon gochujang (hot pepper paste)
- 1 teaspoon toasted sesame oil
- Optional: 2 cups of anchovy stock (or chicken or beef broth)

For stock:

- 7 large dried anchovies, heads and guts removed
- ⅓ cup Korean radish (or daikon radish), sliced thinly
- 4 x 5 inch dried kelp
- 3 green onion roots
- 4 cups water

## Instructions

Anchovy stock (can be made vegan as well):

1. Put the anchovies, daikon, green onion roots, and dried kelp in a sauce pan.
2. Add the water and boil for 20 minutes over medium high heat.
3. Lower the heat to low for another 5 minutes.
4. Strain.

Kimchi stew:

1. Place the kimchi and kimchi brine in a shallow pot. Add pork and onion
2. Slice 2 green onions diagonally and add them to the pot.
3. Add salt, sugar, hot pepper flakes, and hot pepper paste. Drizzle sesame oil over top and add the anchovy stock
4. Kimchi stew (kimchijjigae: 김치찌개)Kimchi stew (kimchijjigae: 김치찌개)
5. Cover and cook for 10 minutes over medium high heat.Kimchi stew (kimchijjigae: 김치찌개)
6. Open and mix in the seasonings with a spoon. Lay the tofu over top.Kimchi stew (kimchijjigae: 김치찌개)Kimchi stew (kimchijjigae: 김치찌개)
7. Cover and cook another 10 to 15 minutes over medium heat.
8. Chop 1 green onion and put it on the top of the stew. Remove from the heat and serve right away with rice.
